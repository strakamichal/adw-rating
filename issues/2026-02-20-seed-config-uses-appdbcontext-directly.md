...
- [ ] Remove the direct `AppDbContext` usage from `SeedConfigCommand`
- [ ] Either expose an `EnsureDatabaseAsync()` method on a repository/service interface, or move database initialization to the `AddDataMssql` extension method
- [ ] Build passes with no direct `AppDbContext` references in `AdwRating.Cli` (other than DI registration namespace import)
